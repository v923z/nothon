...
function remove_diacritics(name) {
	var control_characters = ['\\', '"', '\'', '}', '{']
	for(var d in control_characters) {
		name = name.replace(control_characters[d], '');
	}
	for(var d in diacritics) {
		name = name.replace(diacritics[d], d);
	}
	return name
}

function _generate_bibtex_key(id, bibliography) {
	var suffix = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']
	// Format is not used for the time being
	var author = (bibliography[id].author ? bibliography[id].author : 'random_key')
	var year = (bibliography[id].year ? bibliography[id].year : 9999)
	var first_author = remove_diacritics(author.split(' and ')[0])
	if(first_author.indexOf(',') > -1) {
		// The name is in the format 'Smith, Joe'
		first_author = first_author.split(',')[0]
	} else {		
		// The name is in the format 'Joe Smith' or 'J. H. Smith'
		var fa_split = first_author.split(' ')
		first_author = fa_split[fa_split.length-1]
		// TODO: we still have to treat the case 'Joe Smith Jr.' somehow
	}
	var key = first_author + year
	var keys = new Array()
	for(var i in bibliography) {
		if(i != id) {
	...
	for(var i=0; i < suffix.length; i++) {
		if(keys.indexOf(key + suffix[i]) == -1) return key + suffix[i]
	}
}
